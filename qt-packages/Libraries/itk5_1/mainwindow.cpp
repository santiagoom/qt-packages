...

{
    ...
    //        return EXIT_FAILURE;
    //    }
    const std::string DicomDirectory = "F:\\data\\esdata\\esdata\\experiment\\CTA20140728_DENGXILIAN\\CTA";
    const std::string outputFileName = "CTA20140728_DENGXILIAN_itkqt1.mhd";
    ...
    try {
        ...
        std::cout << std::endl << DicomDirectory.c_str() << std::endl << std::endl;
        ...
        std::cout << outputFileName.c_str() << std::endl << std::endl;
...
int MainWindow::itk_test_simple()
{

    QString  caseDir  = "F:\\data\\esdata\\esdata\\experiment\\CTA20140728_DENGXILIAN\\CTA";
//    const std::string outputFileName = "CTA20140728_DENGXILIAN_itkqt1.mhd";
    QString originMhd = QString("%1\\%2\\origin.mhd").arg(caseDir).arg("step1");
    QFile originMhdFile(originMhd);

    QString CTA = QString("%1\\%2").arg(caseDir).arg("CTA");
...
void MainWindow::dicom_mhd()
{

    QString caseDir = "F:\\data\\esdata\\esdata\\experiment\\CTA20140728_DENGXILIAN";
    QString originMhd = QString("%1\\%2\\origin.mhd").arg(caseDir).arg("step1");
    QFile originMhdFile(originMhd);

    QString CTA = QString("%1\\%2").arg(caseDir).arg("CTA");

    if (!originMhdFile.exists() && QDir(CTA).exists()) {

        if (!QDir(QString("%1\\%2\\").arg(caseDir).arg("step1")).exists())
            QDir("").mkpath(QString("%1\\%2\\").arg(caseDir).arg("step1"));
        ...
        nameGenerator->SetDirectory(CTA.toLocal8Bit().constData());
        std::string seriesIdentifier;
        using FileNamesContainer = std::vector<std::string>;
        FileNamesContainer fileNames;
        fileNames = nameGenerator->GetFileNames(seriesIdentifier);
        ...
        writer->SetFileName(originMhd.toLocal8Bit().constData());
        writer->SetInput(reader->GetOutput());
        ...
        QMessageBox msgBox;
        msgBox.setText("case dir is not right, please check...");
        msgBox.exec();
        return;
    }
}
